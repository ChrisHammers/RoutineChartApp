 ...
 */
@Singleton
class FirestoreRoutineStepSyncService @Inject constructor() {
     ...
     */
    private fun stepDocument(routineId: String, stepId: String) = db
        .collection("routines")
        .document(routineId)
        .collection("steps")
        .document(stepId)
     ...
     */
    suspend fun syncFromFirestore(routineId: String, stepId: String): RoutineStep? {
        return try {
            val document = stepDocument(routineId, stepId)
                .get(Source.SERVER)
                .await()
     ...
     */
    private fun parseStep(data: Map<String, Any>, id: String): RoutineStep {
        val routineId = data["routineId"] as? String
            ?: throw IllegalArgumentException("Missing routineId for RoutineStep $id")
        val orderIndex = (data["orderIndex"] as? Number)?.toInt()
