...
interface CompletionEventDao {
    ...
    suspend fun getByChildAndDay(
        childId: String,
        dayKey: String,
    ): List<CompletionEventEntity>

    @Query("SELECT * FROM completion_events WHERE childId = :childId AND localDayKey = :dayKey ORDER BY eventAt ASC, id ASC")
    fun observeByChildAndDay(
        childId: String,
        dayKey: String,
    ): Flow<List<CompletionEventEntity>>

    @Query(
        "SELECT * FROM completion_events WHERE childId = :childId AND routineId = :routineId AND stepId = :stepId AND localDayKey = :dayKey ORDER BY eventAt ASC, id ASC",
    )
    suspend fun getByStep(
        childId: String,
        routineId: String,
        stepId: String,
        dayKey: String,
