 ...
 */
@Singleton
class FirestoreUserSyncService @Inject constructor() {
     ...
     */
    private fun userDocument(userId: String) = db
        .collection("users")
        .document(userId)
    ...
    suspend fun syncFromFirestore(userId: String): User? {
        return try {
            ...
            val document = userDocument(userId)
     ...
     */
    private fun parseUser(data: Map<String, Any>, id: String): User {
        val familyId = data["familyId"] as? String
            ?: throw IllegalArgumentException("Missing familyId")
        val roleRaw = data["role"] as? String
