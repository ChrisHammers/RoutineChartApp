 ...
 */
class RoomCompletionEventRepository @Inject constructor(
    ...
    override suspend fun getByChildAndDay(childId: String, dayKey: String): List<CompletionEvent> {
        return completionEventDao.getByChildAndDay(childId, dayKey).map { it.toDomain() }
    }

    override fun observeByChildAndDay(childId: String, dayKey: String): Flow<List<CompletionEvent>> {
    ...
    override suspend fun getByFamilyAndDay(familyId: String, dayKey: String): List<CompletionEvent> {
